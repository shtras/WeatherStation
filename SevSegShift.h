...
public:
    ...
    SevSegShift(byte pinDS, byte pinSHCP, byte pinSTCP,
        const byte numberOfShiftRegisters =
            2, // currently const value (not changeable) - maybe in future
        bool isDigitsOnController =
            false // only Segments are on ShiftRegister - DigitPins are connected to controller
    );
    ...
    void begin(byte hardwareConfig, byte numDigitsIn, const byte shiftRegisterMapDigits[],
        const byte shiftRegisterMapSegments[], bool resOnSegmentsIn = false,
        bool updateWithDelaysIn = false, bool leadingZerosIn = false,
        bool disableDecPoint = false);

protected:
    void segmentOn(byte segmentNum) override;
    void segmentOff(byte segmentNum) override;
    void digitOn(byte digitNum) override;
    void digitOff(byte digitNum) override;
...
private:
    ...
    byte _shiftRegisterMapDigits[4];
    byte _shiftRegisterMapSegments[8];
