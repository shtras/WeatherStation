...
{
    auto now = time_us_64();
    if (now - lastSegmentSwitch_ > 200) {
        lastSegmentSwitch_ = now;
        displayedSegmentIdx_ = (displayedSegmentIdx_ + 1) % 8;

        for (int i = 0; i < registerValues_.size(); ++i) {
            auto& registerValues = registerValues_[i];
            const auto& activeSegments = activeSegments_[i];

            auto segmentPin = segmentPins_[displayedSegmentIdx_];
            registerValues = 1 << segmentPin;

            for (int digit = 0; digit < 4; ++digit) {
                if ((activeSegments[digit] & (1 << displayedSegmentIdx_)) == 0) {
